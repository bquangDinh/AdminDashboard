...
var neumorphismSelectDemo = NeumorphismSelect({
...
function showFileInfo(files) {
  ...
  var fileSize = files[0].size / 1000000;
...
var testDropzone = DropZone({
  dropZoneContainer: '#yourDropzone',
  fileInput: '#test-file',
  handleFileCallback: function handleFileCallback(files) {
    showFileInfo(files);
  },
  handleFileChangeCallback: function handleFileChangeCallback(files) {
    showFileInfo(files);
  },
...
$(function () {
  ...
  $('.sidebar-btn').on('click', function (e) {
    var self = this;
    var targetPanelID = $(this).data('panel-id'); //assign this panel to current panale ID
  ...
  $('#collapse-menu-btn').on('click', function (e) {
    var openMenubarButton = $('#open-menubar-btn');
    var menubar = $('#adb-menubar');
    ...
    $('.left-container').css('flex', '0.0001'); //hide menubar

    animateCSSJquery($(menubar), 'animate__slideOutLeft', function () {
      //animation end
      $(menubar).hide(); //show open menubar button on sidebar
  ...
  $('#open-menubar-btn').on('click', function (e) {
    var self = this;
    var menubar = $('#adb-menubar'); //hide open menubar button
    ...
    $('.left-container').css('flex', '1'); //show menubar
  ...
  $('#close-right-container-btn').on('click', function (e) {
    ...
    var rightPanel = $('#adb-right-container').find('.right-panel').first();
